import { EvaluationResultSchema, type EvaluationMode } from "./contract";

function clamp(n: number, lo: number, hi: number) {
  return Math.max(lo, Math.min(hi, n));
}

export function evaluate(text: string, mode: EvaluationMode) {
  const t = text.trim();
  ...
  const market_comment =
    ...
    `${harshPrefix}这句目前对招聘方几乎不可检索：补动作、补指标、补对象。`;

  const rewrite =
    `Rewritten: ${t} (tie metrics → business impact in one sentence).`;
  ...
  const result = {
    ...
    market_comment,
    rewrite,
    meta: { provider: "rule-based-mvp", version: "v0" as const },
  };
